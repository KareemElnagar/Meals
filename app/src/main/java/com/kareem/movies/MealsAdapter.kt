...
import com.kareem.domain.entity.Category
import com.kareem.movies.databinding.CategoryItemBinding
...
class MealsAdapter() : ListAdapter<Category, MealsAdapter.ViewHolder>(CategoryDiffCallback()) {
    ...
    class ViewHolder(private val itemBinding: CategoryItemBinding) : RecyclerView.ViewHolder(itemBinding.root) {
        fun bind(category: Category) {
            itemBinding.title.text = category.strCategory
            itemBinding.subtitle.text = category.strCategoryDescription
            Glide.with(itemBinding.root.context).load(category.strCategoryThumb).into(itemBinding.imageView)
        }
    }
